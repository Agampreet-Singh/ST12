...
########################
    ...
    echo "----------------------------------------------------------------------------------------------"
    print_message "${COLOR_CYAN}" "Please enter the desired output file path (e.g., encrypted_file.gpg):"
    echo "----------------------------------------------------------------------------------------------"
    echo -n "DESIRED= "
    ...
    echo "-----------------------------------------------------------------------"
    print_message "${COLOR_CYAN}" "Please enter a strong password for encryption:"
    echo "-----------------------------------------------------------------------"
    echo -n "PASSWORD= "
    ...
    # Check if the password is strong enough (you may want to customize this check)
    if [ ${#password} -lt 8 ]; then
        print_message "${COLOR_RED}" "Error: Password must be at least 8 characters long."
        exit 1
    fi
    ...
    # Derive a key from the password and salt using a key derivation function (KDF)
    key=$(openssl enc -aes-256-cbc -pass pass:"${password}" -P -S "${salt}" -n 2>&1 | grep key= | cut -d'=' -f2)
    
    # Encrypt the file using OpenSSL
    openssl enc -aes-256-cbc -salt -in "${file_to_encrypt}" -out "${output_file}" -pass pass:"${key}"
